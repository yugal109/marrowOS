...
// Each scan line is zero padded to the nearest 4-byte boundary. RGB values
// are stored backwards, i.e. BGR.
...
// Color table is present only if bits-per-pixel is less than 8; colors are
// ordered by importance, one entry per color.
struct color_table
{
    uint8_t red;
    uint8_t green;
