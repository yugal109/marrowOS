...
struct command_argument *marrowos_parse_command(const char *command, int max)
{
    ...
    root_command = marrowos_malloc(sizeof(struct command_argument));
    if (!root_command)
    {
        goto out;
    }
    ...
    while (token != 0)
    {
        struct command_argument *new_command = marrowos_malloc(sizeof(struct command_argument));
        if (!new_command)
        {
            break;
        }
...
void marrowos_terminal_readline(char *out, int max, bool output_while_typing)
{
    ...
    for (i = 0; i < max - 1; i++)
    {
        char key = marrowos_getkey_block();
...
int marrowos_system_run(const char *command)
{
    ...
    struct command_argument *root_command_argument = marrowos_parse_command(buf, sizeof(buf));
    if (!root_command_argument)
    {
        return -1;
    }

    return marrowos_system(root_command_argument);
}
