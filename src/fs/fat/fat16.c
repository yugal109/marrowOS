...
#define MARROWOS_FAT16_SIGNATURE 0x29
#define MARROWOS_FAT16_FAT_ENTRY_SIZE 0x02
#define MARROWOS_FAT16_BAD_SECTOR 0xFFF7
#define MARROWOS_FAT16_UNUSED 0x00
...
static void fat16_init_private(struct disk *disk, struct fat_private *private)
{
    ...
    private->cluster_read_stream = disk_streamer_new_from_disk(disk);
    private->fat_read_stream = disk_streamer_new_from_disk(disk);
    private->directory_stream = disk_streamer_new_from_disk(disk);
}
...
int fat16_get_total_items_for_directory(struct disk *disk, uint32_t directory_start_sector)
{
    ...
    if (disk_streamer_seek(stream, directory_start_pos) != MARROWOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk *disk)
{
    ...
    if (disk_streamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != MARROWOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != MARROWOS_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
// Convert a space-padded FAT 8.3 name/ext field into a normal C string.
...
static int fat16_get_fat_entry(struct disk *disk, int cluster)
{
    ...
    res = disk_streamer_seek(stream, fat_table_position + (cluster * MARROWOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
int fat16_get_cluster_for_offset(struct disk *disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == MARROWOS_FAT16_BAD_SECTOR || (entry >= 0xFFF0 && entry <= 0xFFF6) || (entry == 0x0000))
        {
            res = -EIO;
            goto out;
        }
...
static int fat16_read_internal_from_stream(struct disk *disk, struct disk_stream *stream, uint16_t cluster, int offset, int total, void *out)
{
    ...
    while (total > 0)
    {
        ...
        res = disk_streamer_seek(stream, starting_pos);
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
    ...
    char tmp_filename[MARROWOS_MAX_PATH];
    for (int i = 0; i < directory->total; i++)
    {
        fat16_get_full_relative_filename(&directory->item[i], tmp_filename, sizeof(tmp_filename));
        if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0)
        {
            // Fount it let's create a new fat_item
